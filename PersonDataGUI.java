...
public class PersonDataGUI {
	...
	ArrayList<Person> originalData = new ArrayList<>();
...
*/
	...
	public void createAndShowGUI() {
		...
		final JDialog resetDialog = new CustomDialog(getResetText(), this, EType.RESET);
	...
	public void ResetTable(JTable table, Person[] people){
	    Person[] originalDataArray = new Person[originalData.size()];
		originalDataArray = originalData.toArray(originalDataArray);
		this.personDataModel = new PersonDataModel(originalDataArray);
		table.setModel(this.personDataModel);
		table.repaint();
	}

	JButton choice = new JButton("Избери");
	ActionListener actionListener = new ActionListener() {
		@Override
		public void actionPerformed(ActionEvent e) {
			choice.addActionListener((ActionListener) e);
	...
	public void filterTable(int intValue, JTable table, Person[] people) {
        originalData.clear();
	    for(Person p : people){originalData.add(p);}
		switch (intValue) {
			case 1:
	...
	private static String getResetText() {
		return "Моля въведете число:";
	}
}
